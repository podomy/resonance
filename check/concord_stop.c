...
// concord_stop asserts a workload stopped on one side of
// a partition stays stopped on all nodes after reunion.
// The tombstone must win over late spec copies everywhere.
...
static int stop_workload(int node, const char* id) {
    ...
    snprintf(cmd, sizeof(cmd),
             "XDG_CONFIG_HOME=/tmp/resonance/node%d "
             "./concord workload stop %s 2>/dev/null",
             node, id);
    fp = popen(cmd, "r");
    if (fp == NULL)
        return (0);
...
int main(void) {
    ...
    if (access("./concord", X_OK) != 0) {
        printf("concord_stop: skip no ./concord\n");
        return (0);
    }
    ...
    while (time(NULL) - start < DEADLINE) {
        ...
        if (phase == 0 && seen3 && hold(&t0, HOLD_UP)) {
            ...
            // Converged pre-condition, then split and stop.
            ask_lists(has, shortid, &tcheck);
            if (has[0] && has[1] && has[2]) {
                if (!stop_workload(0, wid))
                    break;
                phase = 2;
                seen3 = 0;
            }
        } else if (phase == 2) {
            // Diverged truths: gone on 0,1, alive on 2.
            ask_lists(has, shortid, &tcheck);
            if (!has[0] && !has[1] && has[2]) {
                phase = 3;
                seen3 = 0;
            ...
            // Tombstone must win everywhere.
            ask_lists(has, shortid, &tcheck);
            if (!has[0] && !has[1] && !has[2] && seen3) {
                done = 1;
                break;
            }
        }
    ...
    if (!done) {
        fprintf(stderr,
                "concord_stop: fail workload %s on %d %d "
                "%d phase=%d\n",
