...
// concord_divergent asserts a workload submitted on each
// side of a partition shows up on all nodes after reunion.
...
int main(void) {
    ...
    pid_t pids[N];
    int hasA[N], hasB[N];
    char widA[64], widB[64], shortA[16], shortB[16];
    ...
    if (!sim_tuns_open(fds, N)) {
        printf("concord_divergent: skip (%s)\n",
               strerror(errno));
        return (0);
    }
    ...
    while (time(NULL) - start < DEADLINE) {
        ...
        if (!drop && !restored && seen3) {
            if (t0 == 0)
                ...
                seen3 = 0;
                t0 = time(NULL);
                // One workload each side of the split.
                if (submit_workload(ISOLATE, widA,
                                    sizeof(widA))) {
                    memcpy(shortA, widA, 8);
                    shortA[8] = '\0';
                } else {
                    break;
                }
                if (submit_workload(0, widB,
                                    sizeof(widB))) {
                    memcpy(shortB, widB, 8);
                    shortB[8] = '\0';
                } else {
                    break;
                }
            ...
            t0 = time(NULL);
            tcheck = 0;
        } else if (restored && shortA[0] != '\0' &&
                   shortB[0] != '\0') {
            // Ask every node directly every 2s.
            if (tcheck == 0 || time(NULL) - tcheck >= 2) {
                tcheck = time(NULL);
                for (i = 0; i < N; i++) {
                    hasA[i] = workload_present(i, shortA);
                    hasB[i] = workload_present(i, shortB);
                }
                if (hasA[0] && hasA[1] && hasA[2] &&
                    hasB[0] && hasB[1] && hasB[2])
                    break;
            }
        }
        ok = 0;
    ...
    if (!hasA[0] || !hasA[1] || !hasA[2] || !hasB[0] ||
        !hasB[1] || !hasB[2]) {
        fprintf(stderr,
                "concord_divergent: fail A %s on %d %d %d, "
                "B %s on %d %d %d\n",
                shortA, hasA[0], hasA[1], hasA[2], shortB,
                hasB[0], hasB[1], hasB[2]);
        return (1);
    }
    return (0);
