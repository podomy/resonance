...
// concord_scale asserts 5-node mesh, isolate one node,
// reunion: workload and mesh converge on all five.
...
int main(void) {
    ...
    if (access("./bin/concord", X_OK) != 0) {
        printf("concord_scale: skip no ./bin/concord\n");
        return (0);
    }
    ...
    while (time(NULL) - start < DEADLINE) {
        ...
        if (phase == 0 && seen5 && hold(&t0, HOLD_UP)) {
            ...
            // Converged pre-condition, then split.
        ...
        } else if (phase == 2 && hold(&t0, HOLD_UP)) {
            phase = 3;
            tcheck = 0;
        } else if (phase == 3) {
            // Workload on all five, mesh back to 5.
            ask_lists(has, shortid, &tcheck);
            if (has[0] && has[1] && has[2] && has[3] &&
                has[4] && mesh5(&tmesh)) {
                done = 1;
    ...
    if (!done) {
        fprintf(stderr,
                "concord_scale: fail workload %s on "
                "%d %d %d %d %d phase=%d\n",
