...
// concord_minority_stop asserts a workload stopped on
// the lone side of a partition stays stopped on all
// nodes after reunion. The tombstone must win without
// quorum behind it.
...
int main(void) {
    ...
    if (access("./bin/concord", X_OK) != 0) {
        printf("concord_minority_stop: skip no ./bin/concord\n");
        return (0);
    }
    ...
    if (!sim_tuns_open(fds, N)) {
        printf("concord_minority_stop: skip (%s)\n",
               strerror(errno));
        return (0);
    }
    ...
    while (time(NULL) - start < DEADLINE) {
        ...
        if (phase == 0 && seen3 && hold(&t0, HOLD_UP)) {
            ...
            // Converged pre-condition, then split and
            // stop on the lone node.
            ask_lists(has, shortid, &tcheck);
            if (has[0] && has[1] && has[2]) {
                if (!stop_workload(ISOLATE, wid))
                    break;
                phase = 2;
                seen3 = 0;
            }
        } else if (phase == 2) {
            // Diverged truths: alive on 0,1, gone on 2.
            ask_lists(has, shortid, &tcheck);
            if (has[0] && has[1] && !has[2]) {
                phase = 3;
                seen3 = 0;
            ...
            // Lone tombstone must win everywhere, mesh
            // back to 3 alive on all nodes.
            ask_lists(has, shortid, &tcheck);
            if (!has[0] && !has[1] && !has[2] &&
                mesh3(&tmesh)) {
                done = 1;
    ...
    if (!done) {
        fprintf(stderr,
                "concord_minority_stop: fail workload %s "
                "on %d %d %d phase=%d\n",
